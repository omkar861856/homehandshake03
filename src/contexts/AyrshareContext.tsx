...
export function AyrshareProvider({ children }: { children: ReactNode }) {
  ...
  const openSSOLogin = useCallback(
    async (options?: {
      ...
      try {
        const success = await ayrshareService.openSSOLogin(profileKey, options);
        if (!success) {
  ...
  const generateSSOUrl = useCallback(
    async (options?: {
      ...
      try {
        return await ayrshareService.generateSSOUrl(profileKey, options);
      } catch (err) {
        const errorMessage =
          err instanceof Error ? err.message : "Unknown error";
  ...
  }, [user, isLoaded, refreshProfileKey]);
